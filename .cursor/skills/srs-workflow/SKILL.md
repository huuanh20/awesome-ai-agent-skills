...
Phase 7  Auto-Validate        ← .cursor/scripts/srs_validator.py
...
Load `.cursor/skills/srs-workflow/references/brainstorm-guide.md` for domain-specific question sets.
...
Load `.cursor/skills/srs-workflow/references/plan-structure-guide.md`.
...
Load and follow `.cursor/skills/srs-generator/references/srs-template.md` for each section's format.
...
```bash
python .cursor/scripts/srs_validator.py --dir projects/{slug}/srs/
```
